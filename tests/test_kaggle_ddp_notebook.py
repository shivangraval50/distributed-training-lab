"""Checks for notebooks/kaggle_ddp_2gpu.py.
...
SCRIPT_PATH = REPO_ROOT / "notebooks" / "kaggle_ddp_2gpu.py"
...
def test_main_refuses_to_run_without_cuda():
    kaggle_script = importlib.import_module("notebooks.kaggle_ddp_2gpu")
...
def test_main_refuses_to_run_without_exactly_two_gpus(monkeypatch):
    """The "exactly 2 GPUs" guard is a second, distinct check from the
    "no CUDA at all" guard above -- on this machine (no CUDA), the
    no-CUDA branch always short-circuits first, so that second guard is
    never actually exercised by the test above. Monkeypatch only the CUDA
    *detection* functions (not main()'s logic) to simulate "CUDA available,
    but only 1 GPU visible" and confirm the real device_count()!=2 branch
    in main() fires."""
    kaggle_script = importlib.import_module("notebooks.kaggle_ddp_2gpu")
